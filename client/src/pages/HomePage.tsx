...
const HomePage: React.FC = () => {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100vh', overflow: 'hidden' }}>
      ...
      {/* Top Left - Profile */}
      <div style={{ position: 'absolute', top: '1rem', left: '1rem', background: '#fa680075', borderRadius: '1rem', padding: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem', boxShadow: '0 4px 6px rgba(0,0,0,0.3)', cursor: 'pointer' }} onClick={() => setIsEditingProfile(true)}>
        <div style={{ width: '75px', height: '75px', borderRadius: '50%', background: '#e2e8f0', overflow: 'hidden', border: '4px solid #fa6800' }}>
        ...
        <div style={{ paddingRight: '5rem' }}>
          <div style={{ fontWeight: 'bold', fontSize: '1.5rem', color: '#dcd5d5' }}>{nickname || 'Guest'}</div>
          <div style={{ fontSize: '0.8rem', color: '#fa6800', fontWeight: 'bold' }}>Lv. 1</div>
        </div>
      </div>
      ...
      <div style={{ position: 'absolute', bottom: '2rem', left: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }} onClick={() => setShowSettings(true)}>
        <div style={{ width: '60px', height: '60px', background: '#f10f0f', borderRadius: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 10px rgba(0,0,0,0.3)' }}>
      ...
      {/* Center - Cards */}
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', gap: '0rem' }}>
        
        {/* Create Room Card */}
        <div 
          onClick={() => navigate('/create')}
          style={{
            width: '200px', height: '300px', background: '#ffffff71', borderRadius: '1rem', padding: '0.5rem',
        ...
        >
          <div style={{ width: '100%', height: '100%', background: 'linear-gradient(135deg, #ef4444 0%, #991b1b 100%)', borderRadius: '0.5rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '4px solid #1e293b', position: 'relative', overflow: 'hidden' }}>
            <span style={{ fontSize: '3rem', fontWeight: 900, color: 'white', textShadow: '3px 3px 0 #1e293b', transform: 'rotate(-15deg)' }}>CREATE</span>
      ...
      {/* Profile Edit Modal */}
      {isEditingProfile && (
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="glass-panel" style={{ width: '400px', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '0.5rem', color: '#cbd5e1' }}>Avatar</label>
                ...
                {Array.from({ length: 10 }, (_, i) => `profile${i + 1}.png`).map(pic => (
                  <div 
                    ...
                    style={{ 
                      ...
                      background: '#ff9900', overflow: 'hidden'
                    }}
                  >
